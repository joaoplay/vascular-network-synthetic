...
def estimate_flows(graph: nx.Graph,
    ...
    param graph: NetworkX graph with:
        enode attribute 'node_label': [x, y, z] coordinates
...
def _murray_law_radius(abs_flow: float, graph: nx.Graph, min_radius: float) -> float:
    """Estimate radius via Murray's law (r ∝ Q^(1/3)) when pressure data are unreliable.

    Calibrates the proportionality constant against edges in *graph* that
    already carry both ``avgRadiusAvg`` and ``flow`` attributes.  If no such
    edges exist, falls back to *min_radius*.

    Murray's law guarantees that large-flow vessels receive proportionally
    larger radii (r ∝ Q^(1/3)), preventing the situation where a vessel with
    a large radius is incorrectly assigned the minimum capillary flow.

    Args:
        abs_flow: Absolute volumetric flow (nl/min) for the edge being estimated.
        graph: NetworkX graph that may contain reference edges with known radii
            and flows.
        min_radius: Floor radius returned when no reference data are available.

    Returns:
        Estimated radius in microns, at least *min_radius*.
    """
    ref_radii = []
    ref_flows = []
    for _u, _v, data in graph.edges(data=True):
        r = data.get('avgRadiusAvg')
        f = abs(data.get('flow', 0.0) or 0.0)
        if r and float(r) > 0 and f > 0:
            ref_radii.append(float(r))
            ref_flows.append(f)
    ...
    ref_q = float(np.median(ref_flows))

    radius = ref_r * (abs_flow / ref_q) ** (1.0 / 3.0)
...
def compute_radius_from_flow(flow: float, u: int, v: int, graph: nx.Graph,
    ...
    consistent: ``cond = facfp * d^4 / (L * μ)``, ``Q = cond * ΔP``.

    When the pressure drop across the edge is negligibly small (which can
    happen for parallel paths or newly added edges whose pressures are not
    yet solved), the function falls back to Murray's law
    (``r ∝ Q^(1/3)``) calibrated against existing vessels in *graph*.
    This prevents large-flow vessels from being incorrectly assigned the
    minimum capillary radius (~0.04 nl/min equivalent).
    ...
    delta_p = abs(p_u - p_v)
    if delta_p < 1e-10:
        # Pressure data unavailable or degenerate (e.g. parallel paths).
        # Use Murray's law (r ∝ Q^(1/3)) so that large-flow vessels still
        # receive a proportionally large radius instead of min_radius.
        return _murray_law_radius(abs_flow, graph, min_radius)
