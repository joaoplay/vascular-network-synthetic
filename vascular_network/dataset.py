...
from vascular_network.vessap_utils import *


""""
IMPORTANT: PLEASE NOTE THAT THIS CODE WAS NOT WRITTEN BY ME. IT WAS WRITTEN BY THE AUTHORS THIS PAPER AND
 GITHUB REPOSITORY:
...
class VesselGraphDataset(InMemoryDataset):
    ...
    def __init__(self,
        ...
        print("Available Datasets are:", AVAILABLE_DATASETS.keys())

        # check if dataset name is valid
        assert self.name in AVAILABLE_DATASETS.keys()

        self.url = AVAILABLE_DATASETS[self.name]['url']
        self.seed = seed
        self.use_edge_attr = use_edge_attr
        self.use_atlas = use_atlas
        self.number_of_workers = int(number_of_workers)

        self.AlanBrainAtlas = AVAILABLE_DATASETS[self.name]['AlanBrainAtlas']

        super(VesselGraphDataset, self).__init__(root)
    ...
    def raw_file_names(self):
        ...
        folder = osp.join(self.raw_dir, AVAILABLE_DATASETS[self.name]['folder'])
    ...
    def download(self):
        ...
        name = AVAILABLE_DATASETS[self.name]['folder']
    ...
    def process(self):
        ...
        for i in range(len(subfolders)):
            ...
            if self.AlanBrainAtlas and self.use_atlas:
                ...
                data.node_attr_keys = ['pos_x', 'pos_y', 'pos_z', 'degree', 'isAtSampleBorder'] + list(
    ...
    def process(self):
        ...
        for i in range(len(subfolders)):
            ...
            if self.splitting_strategy == 'spatial':  # sample negative edges only in spatial proximity of nodes (mean + 2sigma)
                ...
                data = negative_sampling(data.edge_index_undirected, df_edges, data, data.edge_index_undirected,
                                         n_train, n_test, n_val, self.number_of_workers)
